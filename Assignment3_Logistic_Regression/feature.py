...
class BagOfWordsFeatureExtractor(object):
    ...
    def preprocess(self, documents):
        ...
        for wrd in word_freq:
            if(word_freq[wrd]<self.min_freq):
                remove_words.add(wrd)
...
class TfIdfFeatureExtractor(object):
    ...
    def preprocess(self, documents):
        ...
        ''' YOUR CODE HERE '''
        for wrd in total_word_freq:
            if total_word_freq[wrd]<self.min_freq:
                remove_words.add(wrd)
        # Check frequency of each word and add to 'remove_words'
        ...
        # 'word_freq_tensor'.
        i=0
        for w in doc_word_freq:
            self.word_to_idx[w]=i
            self.idx_to_word[i]=w
            word_freq_tensor[i]=np.log(num_docs/doc_word_freq[w])
            i+=1
