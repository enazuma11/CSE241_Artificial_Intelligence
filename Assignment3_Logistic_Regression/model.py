...
class LogisticRegression(object):
    ...
    def loss(self, X_batch, Y_batch):
        ...
        loss  = ( -np.dot(Y_batch,np.log(pred))-np.dot(1-Y_batch,np.log(1-pred)) ) / Y_batch.size
        grad = np.dot(X_batch.T,pred - Y_batch)/Y_batch.size
        ''' END YOUR CODE HERE '''
    ...
    def train(self,
        ...
        R=X_train.shape[0]
        if self.add_bias:
            self.weights = np.random.randn(C+1) # Fill a correct value here
        else:
            self.weights = np.random.randn(C) # Fill a correct value here
        ...
        # while taking in consideration the bias (so two cases according to the boolean value of add_bias).
        if self.add_bias:
            X_train_adj = np.column_stack((X_train,np.ones(R)))  # Fill a correct value here
        else:
            X_train_adj = X_train

        # Perform iterations, use output of loss function above to get unregularized loss and gradient
        freq = X_train_adj.shape[1]
        for i in range(num_iters):
            loss, grad = self.loss(X_train_adj, Y_train)
            loss  = loss + (reg_const/(2*freq))*np.dot(self.weights.T,self.weights)
            grad=(grad + (reg_const/freq)*self.weights)
